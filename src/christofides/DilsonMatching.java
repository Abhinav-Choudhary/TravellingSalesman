...
public class DilsonMatching {
    ...
    //--------------------------------------Public Methods-----------------------------------------------------------//
    public DilsonMatching(BlossomGraph g) {
        this.g = g;
        // Number of vertices
        n = g.getNodeCount();
        // Number of edges
    ...
    private void initialize() {
        ...
        for (int i=0; i<2*n; i++) {
//        	deep = new ArrayList<>(Collections.nCopies(2*n, new ArrayList<Integer>()));
        	deep.add(new ArrayList<Integer>());
//            shallow = new ArrayList<>(Collections.nCopies(2*n, new ArrayList<Integer>()));
    ...
    private void heuristic() {
        ...
        PriorityQueue<Pair> b = new PriorityQueue<>(new Comparator<Pair>() {
            @Override
        ...
        for(int u = 0; u < n; u++) {
            if(isEdgeBlocked(u)) continue;
            degree.set(u, g.adj(u).size());
        }

        for(int i = 0; i < n; i++)
            b.add(new Pair(degree.get(i), i));
    ...
    private void expand(int u, boolean expandBlocked) {
        ...
        mate.set(it.get(0), mate.get(u));
        pos++;
        //
        // Now we go through the odd circuit adjusting the new mates
        while(pos < shallow.get(u).size()-1) {
            mate.set(it.get(pos), it.get(pos+1));
            mate.set(it.get(pos+1), it.get(pos));
            pos += 2;
        }
    ...
    private int blossom(int u, int v) {
        ...
        for(int ele: shallow.get(t)) {
            u_ = ele;
            outer.set(u_, t);
            for(int jt: deep.get(u_)) {
                deep.get(t).add(jt);
                outer.set(jt, t);
            }
        }
    ...
    public void solveMinimumCostPerfectMatching() {
        solveMaximumMatching();
//        if(!perfect) throw new Exception("Error: The graph does not have a perfect matching");
        clear();
        ...
        System.out.println("Bloosom Edges: " + g.getEdge(matching.get(0)).u + " " + g.getEdge(matching.get(0)).v);
        System.out.println("Bloosom Edges: " + g.getEdge(matching.get(1)).u + " " + g.getEdge(matching.get(1)).v);
